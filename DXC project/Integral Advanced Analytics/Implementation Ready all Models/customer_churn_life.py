...
def run_script_customer_churn_life(server,database,username,password):
    import pandas as pd
    ...
    query_churn_clnt_no = '''select distinct clntnum,
    c.cltsex,
    ...
    case when (select datediff(day,cast(cast(min(occdate) as varchar(8)) as date),getdate())
    from vm1dta.chdrpf pol where pol.COWNNUM = c.CLNTNUM and pol.VALIDFLAG=1) <0 then 10 
    else  (select datediff(day,cast(cast(min(occdate) as varchar(8)) as date),getdate())
    from vm1dta.chdrpf pol where pol.COWNNUM = c.CLNTNUM and pol.VALIDFLAG=1) end elapseddays
    ...
    and c.cltdod = 99999999 
    '''
    #inputDf=pd.read_csv("churnclntnum.csv",dtype={'clntnum':'str'})
    
    inputDf = pd.read_sql(query_churn_clnt_no,cnxn)
    ...
    query_net_profit ='''select clntnum,premium,isnull(claimpaid,0),(premium-isnull(claimpaid,0)) net_profit
    from(
    ...
    premiumDf = pd.read_sql(query_churn_clnt_no,cnxn)
    ...
    #cltvDf['cltvscore']= ((cltvDf['net_profit']*cltvDf[365])/(1+0.12)) + ((cltvDf['net_profit']*cltvDf[730])/(1+0.11)**2)+                     #  ((cltvDf['net_profit']*cltvDf[1095])/(1+0.1)**3)+
    #((cltvDf['net_profit']*cltvDf[1460])/(1+0.09)**4)+ ((cltvDf['net_profit']*cltvDf[1825])/(1+0.08)**5)
    ...
    csvDf=cltvDf.merge(inputDfCoordinates[['client_name','cltsex','marryd','occpcode','latitude','longitude']],
